...
resource "aws_ec2_tag" "cluster_primary_security_group" {
  ...
  resource_id = local.eks_cluster.vpc_config[0].cluster_security_group_id
  key         = each.key
  value       = each.value
}
...
data "tls_certificate" "this" {
  ...
  url = local.eks_cluster.identity[0].oidc[0].issuer
}
...
data "aws_eks_addon_version" "this" {
  ...
  kubernetes_version = coalesce(var.cluster_version, local.eks_cluster.version)
...
resource "aws_eks_identity_provider_config" "this" {
  ...
  oidc {
    ...
    issuer_url                    = try(each.value.issuer_url, local.eks_cluster.identity[0].oidc[0].issuer)
